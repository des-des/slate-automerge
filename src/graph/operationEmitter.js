 ...
 */
export default graph => (slateOperations, value, clientId) => {
  applySlateOperationsHelper(graph, slateOperations, value)
}
 ...
 */
const applySlateOperationsHelper = (graph, operations, value) => {
    operations.forEach(op => {
      console.log(op);
        ...
        let currentNode = value.document;
        switch (op.type) {
            ...
            // console.log({ path, currentNode });
                path.forEach(el => {
                  // console.log({ currentNode: currentNode.toJS(), el });
                  // console.log(currentNode.nodes);
                  // console.log(currentNode.nodes.toJS());
                  // console.log(currentNode.nodes.get(0));
                    currentNode = currentNode.nodes.get(el);
                })
                console.log(currentNode.toJS());
                const id = (currentNode.data && currentNode.data.id) || `BLOCK_${currentNode.key}`
                graph.insertText(id, offset, text)
            ...
            case "insert_node":
                ...
                rest.forEach(el => {
                  ...
                  currentNode = currentNode.blocks[el];
                })
                const nodeId = graph.insertBlock(node, parentNode.data.id)
